...
// Concretely: l_i(0) = ∏_{j≠i} (-x_j) / (x_i - x_j)
//
// All xs must lie in the same field (share the same modulus) and be pairwise distinct, otherwise the denominator
// becomes non-invertible and an error is returned. This is used by InterpolatePolynomialZero and
// InterpolateCommitmentZero to reconstruct values at 0 via Lagrange interpolation.
